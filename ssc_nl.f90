...
INTEGER                :: I,J,N
REAL, DIMENSION(IM,JM) :: U,V,H,UF,VF,HF, FU, FV, adv1, adv2, adv3
...
!--prostorna petlja za H
	DO I = 2,IM-1
		DO J = 2,JM-1
			adv3(I,J) = ( U(I,J) - U(I-1,J) )* ( H(I+1,J) - H(I-1,J) )/(4*d)
			adv3(I,J) = adv3(i,j) + (V(I,J)-V(I,J-1))* (H(I,J+1)-H(I,J-1))/(4*d)
			HF(I,J) = H(I,J) - dt*H(I,J)* ( U(I,J) - U(I-1,J) + V(I,J) - V(I,J-1) )/d -dt*adv3(i,j)
		ENDDO
	ENDDO
...
!--prostorna petlja za brzine	
	DO I = 2,IM-1
		DO J = 2,JM-1
			...
			UF(I,J) = U(I,J) - dtgd* (HF(I+1,J) - HF(I,J)) + FV(I,J) -dt*adv1(i,j)
			VF(I,J) = V(I,J) - dtgd* (HF(I,J+1) - HF(I,J)) - FU(I,J) -dt*adv2(i,j)
		ENDDO
	ENDDO
...
!--sub za ispis u .txt fileove
	open(12, file='h_nl.out')
	open(13, file='u_nl.out')
	open(14, file='v_nl.out')
